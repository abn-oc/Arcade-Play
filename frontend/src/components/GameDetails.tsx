...
export default function GameDetails({ id }: GameDetailsProps) {
  ...
  const [leaderBoard, setLeaderBoard] = useState<{Username: string, Score: number}[]>([{Username: 'loading...', Score: -1}]);
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        setGame(gameDetails);
        const board: {Username: string, Score: number}[] = (await getGameLeaderboard(id));
        console.log(board);
