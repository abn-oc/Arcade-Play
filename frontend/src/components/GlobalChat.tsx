...
export default function GlobalChat() {
    ...
    const [globalMsgs, setGlobalMsgs] = useState<{username: string, content: string}[]>([]);
    ...
    if (globalMsgText.trim()) {
        if (socket && user) socket?.emit('globalMessage', {username: user?.Username, content: globalMsgText});
        setGlobalMsgText('');
        }
    };
    ...
    useEffect(() => {
        ...
        function updateMsgs(msg: {username: string, content: string}) {
            setGlobalMsgs(prev => [...prev, { username: msg.username, content: msg.content }]);
        }
